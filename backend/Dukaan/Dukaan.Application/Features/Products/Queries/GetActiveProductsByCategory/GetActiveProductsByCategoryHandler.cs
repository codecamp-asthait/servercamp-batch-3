...
public class GetActiveProductsByCategoryHandler(IRepository<Product> repository)
    : IQueryHandler<GetActiveProductsByCategoryQuery, ErrorOr<PagedResponse<ProductDto>>>
{
    public async Task<ErrorOr<PagedResponse<ProductDto>>> Handle(GetActiveProductsByCategoryQuery request, CancellationToken cancellationToken)
    {
