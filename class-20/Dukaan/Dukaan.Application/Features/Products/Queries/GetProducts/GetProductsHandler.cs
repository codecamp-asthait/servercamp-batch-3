...
public class GetProductsHandler(IRepository<Product> repository)
    : IQueryHandler<GetProductsQuery, ErrorOr<PagedResponse<ProductDto>>>
{
    public async Task<ErrorOr<PagedResponse<ProductDto>>> Handle(GetProductsQuery request, CancellationToken cancellationToken)
    {
