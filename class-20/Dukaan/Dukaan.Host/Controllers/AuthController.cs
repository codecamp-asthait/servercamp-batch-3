...
{
    [HttpPost("login")]
    public async Task<IActionResult> LoginWithJwt(Application.Dtos.LoginRequestDTO request)
    {
        try
        {
    ...
    public async Task<ActionResult<Application.Features.Auth.Dtos.CustomerAuthResponse>> CustomerLogin(
    [FromHeader(Name = "x-tenant-slug")] string tenantSlug,
    Application.DTOs.CustomerLoginRequest request)
    {
        ...
        var response = await Mediator.Send(new CustomerLoginCommand(request.Email, request.Password, tenantId.Value));
